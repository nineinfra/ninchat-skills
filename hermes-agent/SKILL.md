...
> **用户权限说明**：
> | 用户类型 | 搜索级别 | 频率限制 | 结果条数上限 |
> |---------|---------|---------|------------|
> | **匿名用户** | basic（摘要） | 10 次/分钟 | 50 条 |
> | **普通用户** | basic（摘要） | 60 次/分钟 | 100 条 |
> | **群组用户** | **full（全文）** | 120 次/分钟 | 200 条 |
>
> 💬 **群组用户**：扫描下方二维码加入微信群，即可获得 full 搜索级别（可查看完整文章内容）、更高频率和更多结果。
>
> ![微信群二维码](../wechat_group.png)
...
```bash
curl -s "https://ninchat-api.cpolar.top/api/ai-hot-comments?limit=10&days=7"
```
